...
_.extend(Pillbox.prototype, {
  ...
  addPill: function (pillOptions) {
    ...
    // each time we add a pill, see if the pillbox has grown too tall.
    // if we're not in expanded mode, trigger a collapse
    if (!this.expanded && this.el.scrollHeight > this.collapsedHeight) {
      this.collapse();
    }
  ...
  expand: function () {
    if (this.el.scrollHeight > this.collapsedHeight) {
      this.animate(this.el, {
        height: Math.min(this.expandedHeight, this.el.scrollHeight)
      });

      // switch "is-collapsed" class to "is-expanded"
      this.el.className = _.filter(this.el.className.split(" "), function (name) {
        return name !== "is-collapsed";
      }).join(" ");
      this.el.className += " is-expanded";
  ...
  collapse: function () {
    this.animate(this.el, {
      height: Math.min(this.collapsedHeight, this.el.scrollHeight)
    });

    // switch "is-expanded" class to "is-collapsed"
    this.el.className = _.filter(this.el.className.split(" "), function (name) {
      return name !== "is-expanded";
    }).join(" ");
    this.el.className += " is-collapsed";
