...
function Pillbox(options) {
  ...
  if (!options.container) { throw new Error('A DOM container is required.'); }
...
Pillbox.prototype = _.create(EventEmitter.prototype, /** @lends Pillbox.prototype */ {
  /**
  ...
  addPill: function (pillOptions) {
    ...
    pill.on("request:remove", _.bind(function () {
       ...
       */
      this.emit("pill:request:remove", { pill: pill });
    }, this));
    ...
    pill.on("click", _.bind(function () {
       ...
       */
      this.emit("pill:click", { pill: pill });
    }, this));
  ...
  removePill: function (key) {
    if (!this._pills[key]) { return; }
    this._pills[key].erase();
    delete this._pills[key];
  ...
  /**
   * Return an array of all keys that reference the current collection of pills.
   * @return {String[]}
   */
  keys: function () {
    return _.keys(this._pills);
  },

  /**
   * Retrieve an instance of a pill, given its key.
   * @param {String} key The key used to look up the pill
   * @return {Object} A pill instance
   */
  getPill: function (key) {
    return this._pills[key];
  }
});
